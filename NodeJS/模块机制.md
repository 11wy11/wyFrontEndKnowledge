...
### 优先从缓存加载
Node对引入的模块都会进行缓存，而且缓存的是编译执行后的对象。
...
实际上 .node模块并不需要编译，因为它是编写C/C++模块之后编译生成的，所以这里只有加载和执行的过程。在执行的过程中，模块exports对象与.node模块产生联系，然后返回给调用者。
...
### commonJs 输出时的危险操作
其实上个例子中的 const { foo } = require('./a.js') 或者 const foo = require('./a.js').foo 写法是相当危险的。因为commonJs输出的值的拷贝，若后面在a.js中 对foo的内存指向作出改动，则不能及时更新。
我们将上面的栗子做个小改动：
```js
/*************** a.js**********************/
const foo = {
...
参考链接：https://www.cnblogs.com/dasusu/p/10105433.html
...
2. 模块可以多次加载，但是只会在第一次加载时运行一次，然后运行结果就被缓存了，以后再加载，直接读取缓存结果，要想让模块再次运行，必须清除缓存。
3. 模块加载顺序，按照代码中出现的顺序

CommonJS能实现控制作用域，模块依赖，模块通信，本质是将模块内的代码放在一个函数内执行，过程中会创建一个对象Module，然后将模块的相关信息包括对外接口信息都添加到对象Module中，供其他模块使用

AMD和CMD用在浏览器端

②AMD是RequireJS在推广过程中对模块定义的规范化产出，

特点：提前执行（异步加载：依赖先执行）+延迟执行

- 定义一个模块时通过`define`命令，通过return声明模块对外暴露的接口

- 依赖其他模块时，通过`require`命令

③CMD（Common Module Definition）是SeaJS在推广过程中对模块化定义的规范化产出

特点：延迟执行（运行到需加载，根据顺序执行）

- 定义一个模块时通过`define`命令，通过`exports`声明模块对外暴露的接口

- 依赖其他模块时，通过`require`命令,同步加载依赖的模块，但也可使用`require.async`来异步加载依赖的模块

④ES6标准

前两种都需要在运行期间才会动态的创建出某个模块，并暴露模块的相关接口。

ES6在代码解析阶段，就能够确定模块以及模块对外的接口，而不用等到运行期。可以让开发工具智能的提示依赖的模块都对外提供了哪些接口
