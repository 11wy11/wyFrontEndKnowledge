...
1. Function允许在运行时动态地创建并变异函数

2. 每次调用Function都会解析函数体，创建新的函数对象，再循环中多次中构造函数会影响执行效率，但是循环中的嵌套函数或者函数定义表达式不会每次重新编译

3. Function创建的函数不是使用词法作用域，函数体代码的编译总会在顶层函数执行。

   在实际编程中很少用到

### 函数式编程

1使用函数处理数组，常用map,reduce

2.高阶函数

3.不完全函数，使用bind返回一个新函数

4.创建私有命名空间的函数通常定义一个立即执行函数
